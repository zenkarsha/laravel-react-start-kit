...
type WordingData = string | number | boolean | WordingData[] | { [key: string]: WordingData };
...
export const _w = (...path: (string | string[])[]): WordingData | undefined => {
  ...
  if (typeof value === "string") {
    ...
    const proto = {
      ...
      param: (key: string, replacedValue: string) => param(value as string, key, replacedValue),
    };
    (value as any).__proto__ = proto;
  }

  if (typeof value === "object" && value !== null && !Array.isArray(value)) {
    value = decodeArrowValueFromObject(value as Record<string, WordingData>);
  }

  return value;
};

const decodeArrowValueFromObject = (obj: Record<string, WordingData>): Record<string, WordingData> => {
  Object.keys(obj).forEach((key) => {
    const val = obj[key];
    obj[key] = decodeArrowValue(val);
    if (typeof obj[key] === "object" && obj[key] !== null && !Array.isArray(obj[key])) {
      obj[key] = decodeArrowValueFromObject(obj[key] as Record<string, WordingData>);
    }
  });
  return obj;
};
...
export const nl2p = (str: string | null | undefined): ReactNode[] | null => {
  ...
  return (
    str?.split?.("\n").map((item, key) => {
      const content: ReactNode = item !== "" ? item : <br key={`br-${key}`} />;
